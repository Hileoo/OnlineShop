 ...
 * customer's request to decrease the number of Products of a title in the cart
 *
 */
public class DecreaseQuantityController extends HttpServlet {
    ...
    public void doPost (HttpServletRequest request, HttpServletResponse response)
        ...
        for (int i = 0; i < cart.getProductCount(); i++) {
            if (cart.getProductAt(i).getProduct_id().equals(Product_id) && cart.getProductAt(i).getQuantity() > 1) {
